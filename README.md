...
`CADDYTOWER_MASTER_KEY`, patches the compose file with the host Docker socket
group for the non-root controller container, and starts bundled `shared-caddy`
/ `watchtower` only when they are missing, including existing compose-managed
containers with generated names.
